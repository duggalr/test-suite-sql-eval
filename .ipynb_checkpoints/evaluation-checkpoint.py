...
def print_scores(scores, etype, model_name, include_turn_acc=True):
    ...
    for level in levels:
      ...
      # save_score('exec', level, exec_level_score, level_count, model_name)
 
      em_level_score = scores[level]['exact']
      print('em-ls:', em_level_score, level)
      # save_score('exact', level, em_level_score, level_count, model_name)
...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint, model_name):
    ...
    print_scores(scores, etype, model_name, include_turn_acc=include_turn_acc)
